...
type SurveyQuestion = { id: number; text: string };
...
export default function SurveyForm({
    ...
    const [answers, setAnswers] = useState(
        questions.map(q => ({ question_id: q.id, text: "" }))
    );

    const handleChange = (i: number, value: string) => {
        setAnswers(prev => prev.map((ans, idx) =>
            idx === i ? { ...ans, text: value } : ans
        ));
    };
    ...
    return (
        <div className="flex flex-col items-center min-h-screen bg-stone-900 text-white px-4 py-12">
            ...
            {questions.map((q, i) => (
                <div key={q.id} className="w-full max-w-4xl mb-10">
                    <p className="font-bold text-2xl mb-1">{i + 1}) {q.text}</p>
