...
function sanitizePath(name: string) {
    ...
    return n.split('/').filter(s => s && s !== '.' && s !== '..').join('/')
}
...
export async function addZipEntriesToUppy(uppy: Uppy, zipBlob: Blob) {
    //Initialize Reader
    const reader = new ZipReader(new BlobReader(zipBlob), { useWebWorkers: true })
    ...
    const basename = (p: string) => p.replace(/\\/g, '/').split('/').pop() || 'file'
    ...
    try {
        ...
        //Check Size
        const totalUncompressed = entries.reduce((sum, e) => sum + (e.uncompressedSize || 0), 0)
        if (entries.length > MAX_FILES || totalUncompressed > MAX_UNCOMPRESSED) throw new Error('Too big')
        ...
        for (const entry of entries) {
            ...
            if ((entry.uncompressedSize ?? 0) > MAX_PER_FILE) throw new Error('Single file too large')

            const name = basename(safePath) //strip leading dir
            const type = guessMime(name) //get mime type

            const blob = await entry.getData(new BlobWriter(type), { useWebWorkers: true }) //get blob
            await uppy.addFile({ name: name, type, data: blob, meta: { originalPath: safePath } }) //add file to uppy
    ...
    useEffect(() => {
        const onAddFiles = async (file: any) => {
            ...
            try {
                ...
                uppy.getPlugin('Dashboard')?.setOptions({ note: `Extracted ${file.name}` })
            } catch (e) {
                ...
                uppy.setOptions({
                    restrictions: { ...(uppy.opts.restrictions ?? {}), allowedFileTypes: prevAllowed },
